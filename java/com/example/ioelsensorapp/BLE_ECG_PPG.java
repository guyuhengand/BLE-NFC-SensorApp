...
public class BLE_ECG_PPG extends AppCompatActivity {

    private static final int ENABLE_BLUETOOTH_REQUEST_CODE = 1;
    private static final int GATT_MAX_MTU_SIZE = 230;

    private static final int CREATE_FILE_ECG = 2;
    private static final int CREATE_FILE_PPG = 3;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_ble_ecg_ppg);
        setTitle("ECG + PPG + BP (BLE)");
        ...
        ppg_chart = findViewById(R.id.chart_ppg);
        ppg_chart.setDrawGridBackground(false);
        ppg_chart.setBackgroundColor(Color.WHITE);
        ppg_data = new LineData();
        ppg_chart.setData(ppg_data);
    }
            ...
            case CREATE_FILE_PPG:
                if (data != null) {
                    ...
                    try {
                        ...
                        ParcelFileDescriptor pfd = getContentResolver().openFileDescriptor(data.getData(), "w");
                        FileOutputStream fileOutputStream = new FileOutputStream(pfd.getFileDescriptor());
                        fileOutputStream.write(dataGenerator(ppg_data).getBytes());
                        fileOutputStream.close();
                        pfd.close();
                        fileOutputStream.close();
                    } catch (Exception e) {
    ...
    private void readECG(BluetoothGattCharacteristic characteristic) {
        log_data.setText(Arrays.toString(arrangestringecg(characteristic.getValue())));
        int[] value = arrangestringecg(characteristic.getValue());
    ...
    private void readPPG(BluetoothGattCharacteristic characteristic) {
        int[] value = arrangestringppg(characteristic.getValue());

        ppg_data = ppg_chart.getData();
        if (ppg_data != null) {
            ILineDataSet set = ppg_data.getDataSetByIndex(0);
            if (set == null) {
                set = createSetppg();
                ppg_data.addDataSet(set);
            }
            for (int j=0; j<36; j++) {
                ppg_data.addEntry(new Entry(set.getEntryCount(), value[j]), 0);
            }
            ppg_data.notifyDataChanged();
            ppg_chart.notifyDataSetChanged();
            ppg_chart.setVisibleXRangeMaximum(250);

        }
        ppg_chart.moveViewToX(ppg_data.getXMax());
    }
    ...
    private void writeStorageAccessFrameworkFile(Context context) {
        ...
        String filename_ecg = "ecg_"+ date_now +".csv";
        String filename_ppg = "ppg_"+ date_now +".csv";
        Intent intent_ecg = new Intent(Intent.ACTION_CREATE_DOCUMENT);
        intent_ecg.addCategory(Intent.CATEGORY_OPENABLE);
        intent_ecg.setType("application/csv");
        intent_ecg.putExtra(Intent.EXTRA_TITLE, filename_ecg);
        startActivityForResult(intent_ecg, CREATE_FILE_ECG);
        Intent intent_ppg = new Intent(Intent.ACTION_CREATE_DOCUMENT);
        intent_ppg.addCategory(Intent.CATEGORY_OPENABLE);
        intent_ppg.setType("application/csv");
        intent_ppg.putExtra(Intent.EXTRA_TITLE, filename_ppg);
        startActivityForResult(intent_ppg, CREATE_FILE_PPG);
    ...
    private int[] arrangestringecg(byte[] aux) {
        int[] str = new int[36];
    ...
    private int[] arrangestringppg(byte[] aux) {
        int[] str = new int[36];
        for (int ij=3; ij<=213; ij+=6) {
            StringBuilder output = new StringBuilder("");
            output.append(((char) aux[ij]));
            output.append(((char) aux[ij+1]));
            output.append(((char) aux[ij+2]));
    ...
    private LineDataSet createSetecg() {
    ...
    private LineDataSet createSetppg() {
